/* ZEPHENHEL CITADEL — Minimal Command UI
   Uses your VERIFIED splitter contract method:
...
function setNetChip(chainIdHex) {
  $("netChip").innerHTML = `BSC: <b>${chainIdHex === BSC_CHAIN_ID_HEX ? "YES" : "NO"}</b>`;
}
...
async function connect() {
  setWarn("");
  if (!window.ethereum) { setWarn("MetaMask not detected."); return; }
  await ensureBsc();
...
async function loadTokenMeta() {
  ...
  if (!provider || !tokenAddr || !ethers.utils.isAddress(tokenAddr)) {
  ...
  try {
    ...
    const price = await fetchDexPriceUsd_BSC(tokenAddr);
    ...
    setWarn("Could not load token meta. Check token address and that you are on BSC.");
  }
}
...
function buildArrays() {
  ...
  for (const r of recs) {
    ...
    shares.push(Math.round(r.pct)); // splitter uses weights, percent weights are fine
  }
...
async function approve() {
  ...
  try {
    ...
    // If allowance already enough, skip
...
function boot() {
  ...
  $("switchBtn").onclick = async () => { try { await ensureBsc(); } catch(e){ setWarn(e?.message||String(e)); } };
  $("addBtn").onclick = () => addRow("", "");
  $("approveBtn").onclick = approve;
  $("splitBtn").onclick = executeSplit;
  $("copyBtn").onclick = copyReceipt;

  $("token").addEventListener("change", loadTokenMeta);
  ...
  if (window.ethereum) {
    ...
    window.ethereum.on("chainChanged", (cid) => setNetChip(cid));
  }

...
(() => {
  ...
  const el = {
    ...
    sn_symbol: $("sn_symbol"),
    sn_symbol2: $("sn_symbol2"),
    vn_symbol: $("vn_symbol"),
    vn_symbol2: $("vn_symbol2"),
    ...
    footerContracts: $("footerContracts"),
  };
  ...
  function pulseRadar() {
    if (!el.radar) return;
    el.radar.classList.remove("pulse");
    void el.radar.offsetWidth;
    el.radar.classList.add("pulse");
  }
  ...
  async function syncChainFromWallet() {
    ...
    el.sn_symbol.textContent = cfg.nativeSymbol;
    el.sn_symbol2.textContent = cfg.nativeSymbol;
    el.vn_symbol.textContent = cfg.nativeSymbol;
    el.vn_symbol2.textContent = cfg.nativeSymbol;

    el.footerContracts.textContent =
      `TokenSplitter: ${cfg.tokenSplitter} • TokenVault: ${cfg.tokenVault} • NativeSplitter: ${cfg.nativeSplitter} • NativeVault: ${cfg.nativeVault}`;
  }
  ...
  async function initContracts() {
    ...
    // Fee policy read from token splitter (canonical for UI)
    try {
    ...
    el.teleFee.textContent = `${feePct}% → ${feeWallet ? feeWallet : "feeWallet"}`;
  ...
  async function stApprove() {
    setError("");
    if (!signer || !userAddress) return setError("LINK WALLET first.");
  ...
  async function snMaxGasSafe() {
    setError("");
    if (!provider || !userAddress) return setError("LINK WALLET first.");
    ...
    try {
      ...
      el.sn_maxHint.textContent = `Max set: ${amt} ${cfg.nativeSymbol} (gas-safe)`;
      setStatus(`MAX SET. Reserved ${ethers.utils.formatEther(reserve)} ${cfg.nativeSymbol}`);
      sonarPing();
    } catch (e) {
  ...
  async function vtCreate() {
    ...
    try {
      ...
      // Attempt to extract vaultId from events if present
      let createdId = null;
      try {
        for (const ev of receipt.events || []) {
          if (ev.args && ev.args.vaultId != null) {
      ...
      setStatus(`TOKEN VAULT CREATED ✅${createdId ? ` (Vault ID: ${createdId})` : ""}`);
      pulseRadar();
      sonarPing();
  ...
  async function vtLoad() {
    setError("");
    if (!provider) return setError("LINK WALLET first.");
  ...
  async function vtPing() {
    setError("");
    if (!signer) return setError("LINK WALLET first.");
    const id = clampInt(el.vt_vaultId.value || "0", 0, 10_000_000);
    try {
  ...
  async function vnCreate() {
    ...
    try {
      ...
      // Attempt to read vaultId from events if present
      let createdId = null;
      try {
        for (const ev of receipt.events || []) {
          if (ev.args && ev.args.vaultId != null) {
      ...
      setStatus(`NATIVE VAULT CREATED ✅${createdId ? ` (Vault ID: ${createdId})` : ""}`);
      pulseRadar();
      sonarPing();
  ...
  async function vnLoad() {
    ...
    try {
      ...
      startVaultTimer("native", id, v.heartbeat, v.inactivityPeriod, v.executed, v.frozen, v.executor, v.balance);
  ...
  function startVaultTimer(kind, vaultId, heartbeat, inactivity, executed, frozen, executorAddr, balanceWei) {
    stopTimer();
    ...
    function paint() {
      ...
      const status = executed ? "EXECUTED" : (unlocked ? "UNLOCKED" : "LOCKED");

      const header = `Vault #${vaultId} • ${kind.toUpperCase()} • ${frozen ? "FROZEN" : "ACTIVE"} • ${status}`;
      ...
      const extra = executorAddr ? `Executor: ${executorAddr}` : "";
      const balLine = (balanceWei != null && kind === "native")
        ? `Vault balance: ${ethers.utils.formatEther(balanceWei)} ${activeCfg().nativeSymbol}`
        : "";
      ...
      // ping radar when it flips to unlocked (once)
      if (!executed && unlocked && remaining > -2) {
        pulseRadar();
        if (el.chkSound?.checked) sonarPing();
      }
    }
    ...
    if (window.ethereum) {
      window.ethereum.on("accountsChanged", async (accs) => {
        ...
        if (!accs || !accs.length) {
          ...
          el.pillWallet.textContent = "NODE: —";
          el.btnConnect.textContent = "LINK WALLET";
  ...
  async function boot() {
    try {
      ...
      setTab("split-token");
      updateGuards();
      setStatus("CITADEL ONLINE. LINK WALLET to begin.");

      // silent connect if authorized
      if (window.ethereum) {
        await ensureProvider();
